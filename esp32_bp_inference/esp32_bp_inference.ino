...
void loop() {
  ...
  // Get predictions
  float systolic = output->data.f[0];
  float diastolic = output->data.f[1];

  // Clamp to reasonable values
  systolic = constrain(systolic, 70, 200);
  diastolic = constrain(diastolic, 40, 130);
  ...
  char bpStr[12];
  snprintf(bpStr, sizeof(bpStr), "%.0f/%.0f", systolic, diastolic);
  // Centre the text horizontally
  int16_t bx, by;
  uint16_t bw, bh;
  display.getTextBounds(bpStr, 0, 0, &bx, &by, &bw, &bh);
  ...
  display.printf("HR:%.0fbpm  SpO2:%.0f%%", heartRate, spo2);

...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, 'extracted_features_v3.csv')
    ...
    for file in csv_files:
        ...
        try:
            ...
            for w in range(num_windows):
                ...
                # Calculate Pulse Width (Half-Prominence)
                widths, _, _, _ = signal.peak_widths(ir_filtered, peaks, rel_height=0.5)
                ...
                features_list.append({
                    ...
                    'ir_ratio': f"{ir_ratio:.4f}",
                    'red_ratio': f"{red_ratio:.4f}",
                    'crest_time_ratio': f"{crest_time_ratio:.4f}",
                    'rise_time_ms': int(mean_rise_time),
                    'heart_rate': int(heart_rate),
                    'pulse_width_ms': int(mean_pulse_width_ms),

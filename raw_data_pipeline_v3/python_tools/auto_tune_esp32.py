...
OUTPUT_DIR     = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'esp32_bp_inference_v3_autotune'))
...
# RF (dari sangat ringan ke ekstrim untuk melihat batas ESP32 dan overfitting)
for n_trees in [5, 10, 20, 50, 100, 150, 200, 250, 300]:
...
tmp_file = os.path.join(OUTPUT_DIR, '_auto_tune_tmp.h')

print(f"{'No':>3} {'Model':<28} {'CV Acc':>7} {'TrainAcc':>8} {'Size':>8} {'Status'}")
print("-" * 75)
...
for i, cfg in enumerate(configs, 1):
    try:
        ...
        # CV Score with Train Score
        scores = cross_validate(pipe, X, y, groups=groups, cv=sgkf, scoring='accuracy', return_train_score=True)
        ...
        print(f"{i:>3} {cfg['name']:<28} {cv_mean*100:>6.1f}% {train_mean*100:>7.1f}% {size_kb:>6.1f}KB {status}")
...
df_results = pd.DataFrame([{
    ...
    'CV_Acc': r['cv_mean'],
    'CV_Std': r['cv_std'],
    'Train_Acc': r['train_mean'],
    'Size_KB': r['size_kb'],
    'Fits': r['fits']
} for r in results])

df_fits = df_results[df_results['Fits']].sort_values('CV_Acc', ascending=False)

print(f"\n{'='*75}")
print("HASIL AUTO-TUNING (DIURUTKAN BERDASARKAN AKURASI CV TERTINGGI & MUAT ESP32)")
print(f"{'='*75}")
print(f"{'Rank':<5} {'Model':<28} {'CV Acc':<10} {'Train Acc':<10} {'Size':<10}")
print(f"{'-'*75}")

for i, (idx, row) in enumerate(df_fits.head(10).iterrows(), 1):
    print(f"{i:<5} {row['Model']:<28} {row['CV_Acc']*100:.1f}%      {row['Train_Acc']*100:.1f}%      {row['Size_KB']:.1f} KB")

/*
 * ESP32 Blood Pressure Classification v3 (Physiological Sandbox)
 *
...
// Include the v3 SVM model exported in the sandbox
#include "svm_model_v3.h"
...
void setup() {
  ...
  if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_I2C_ADDR)) {
    ...
    display.println("BP Sandbox v3");
    display.setCursor(4, 12);
    display.println("Physiological SVM");
    display.setCursor(0, 28);
    display.println("Sensor : MAX30102");
    display.setCursor(0, 40);
    display.println("Model  : SVM Linear");
  ...
  Serial.println("====================================");
  Serial.println("ESP32 Blood Pressure Classifier v3");
  ...
  Serial.println("\n2. SVM Model v3 Loaded (Physiological Features)");
...
void loop() {
  ...
  if (heartRate < 40.0f || heartRate > 180.0f || riseTimeMs <= 0.0f) {
    Serial.println("\n[SQA REJECTED] Sinyal v3 tidak valid!");
  ...
  // Parameter StandardScaler (Z-Score Normalization)
  float mean_ir_ratio = 0.400491f;
  float std_ir_ratio  = 0.191633f;
  float mean_red_ratio = 0.176177f;
  float std_red_ratio  = 0.084899f;
  float mean_heart_rate = 76.784615f;
  float std_heart_rate  = 7.445168f;
  float mean_crest_time_ratio = 0.265718f;
  float std_crest_time_ratio  = 0.023677f;

  float features[4] = {
    (irRatio - mean_ir_ratio) / std_ir_ratio,
    (redRatio - mean_red_ratio) / std_red_ratio,
    (heartRate - mean_heart_rate) / std_heart_rate,
    (crestTimeRatio - mean_crest_time_ratio) / std_crest_time_ratio
  };
  ...
  Serial.println("========================================");
  Serial.printf("Klasifikasi v3  : %s\n", category);
  ...
  display.printf("BPM : %.1f | CT: %.3f", heartRate, crestTimeRatio);

...
# sandbox saves directly to dataset_sandbox
DATASET_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'dataset_sandbox')
...
def record_raw_session():
    # Prompt for Subject Initials BEFORE recording
    print("\n" + "="*50)
    print("側 IDENTITAS SUBJEK")
    while True:
        subject_initial = input("Masukkan Inisial Subjek (Contoh: KYS, AMR, BAPAK): ").strip().upper()
        if not subject_initial:
            print("笶・Inisial tidak boleh kosong!")
        elif '_' in subject_initial:
            print("笶・Inisial tidak boleh mengandung garis bawah (_).")
        else:
            break
    print("="*50)
    ...
    try:
        ...
        print("\n" + "="*50)
        print(f"藤 CONNECTED! Please stay still.")
        print(f"竢ｱ・・Recording {RECORDING_TIME_SECONDS} seconds of raw PPG data...")
        print("="*50 + "\n")
        ...
        while (time.time() - start_time) < RECORDING_TIME_SECONDS:
            ...
            print(f"\rRecording: [{'笆・ * progress}{'.' * (20 - progress)}] {elapsed:.1f}s", end="")
            
        print("\n\n笨・Recording Complete!")
        ser.close()
        ...
        print("\n" + "="*50)
        print("ｩｺ MANUAL LABELING REQUIRED")
        ...
        while True:
            ...
            try:
                ...
                print("笶・Format salah! Harap masukkan dengan format Sistolik/Diastolik (contoh: 120/80).")
                print("庁 Atau ketik 'X' jika Anda benar-benar batal menyimpan datanya.")
            
        filename = f"raw_data_{subject_initial}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_bp{systolic}_{diastolic}.csv"
        ...
        print(f"\n脂 SUCCESS: Data securely saved to {filepath}")
...
if __name__ == "__main__":
    while True:
        print("\n[MAIN MENU sandbox - AUTO SUBJECT]")

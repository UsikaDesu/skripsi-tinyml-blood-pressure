"""
Visualisasi Pipeline Sinyal PPG sandbox
===================================
...
Cara pakai:
  python visualize_signal_sandbox.py                          # File terbaru
  python visualize_signal_sandbox.py raw_data_KYS_..._bp130_74.csv   # File spesifik
  python visualize_signal_sandbox.py raw_data_KYS_..._bp130_74.csv 1 # Window ke-2
"""
import os
import sys
import glob
...
RAW_DATA_DIR = os.path.abspath(os.path.join(SCRIPT_DIR, '..', 'dataset_sandbox'))
...
def visualize_pipeline(filepath, window_idx=0):
    ...
    sqa_status = "✁ELOLOS SQA" if sqa_pass else "❁EDITOLAK SQA"
    ...
    title = f"Pipeline Sinyal PPG  E{subj} ({date_fmt} {time_fmt})  EBP: {sys_bp}/{dia_bp} [{bp_class}]  EWindow {window_idx+1}/{num_windows}"
    ...
    ax4.set_title(f'④ Hasil Akhir: Deteksi Puncak & Rise Time  E{sqa_status}', 
    ...
    print(f"  Time CV (SQA) : {time_cv:.1f}% {'✁E if time_cv <= 15 else '❁E}")
    print(f"  Amp CV (SQA)  : {amp_cv:.1f}% {'✁E if amp_cv <= 25 else '❁E}")
...
def interactive_menu():
    ...
    while True:
        # ── PILIH SUBJEK ──
        print("\n" + "=" * 60)
        print("  VISUALISASI PIPELINE SINYAL PPG sandbox")
        print("=" * 60)
        ...
        print(f"\n>>> Menampilkan: {os.path.basename(filepath)}  EWindow {win_idx+1}")

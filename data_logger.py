                ...
                if line and not line.startswith('timestamp'):
                    try:
                        ...
                        if len(parts) >= 8:
                            ...
                            # Count stable valid readings and fill buffer
                            if valid:
                                stable_count += 1
                            ...
                            streak_info = f" (Streak: {stable_count}/3)" if valid else " (Streak: 0/3)"
                            
                            print(f"Sample {sample_count:4d} | HR: {hr:6.1f} bpm | SpO2: {spo2:5.1f}% | {status}{streak_info}")
                            
                            # Prompt for BP reading after 3 stable samples
                            if stable_count >= 3 and (time.time() - last_prompt_time) > 10:
                                print("\n" + "!" * 60)
                                print("📊 STABLE READING - TIME TO MEASURE BP!")

...
// Include the V2 SVM model exported in the sandbox
#include "svm_model_v2.h"
...
void setup() {
  ...
  Serial.println("\n2. SVM Model V2 Loaded (Physiological Features)");
...
void loop() {
  ...
  if (heartRate < 40.0f || heartRate > 180.0f || riseTimeMs <= 0.0f) {
    Serial.println("\n[SQA REJECTED] Sinyal V2 tidak valid!");
  ...
  float mean_rise_time_ms = 207.948308f;
  float std_rise_time_ms  = 26.584099f;
  float mean_heart_rate = 77.333846f;
  float std_heart_rate  = 7.532952f;

  float features[4] = {
    (irRatio - mean_ir_ratio) / std_ir_ratio,
    (redRatio - mean_red_ratio) / std_red_ratio,
    (riseTimeMs - mean_rise_time_ms) / std_rise_time_ms,

...
// Include the V4 SVM model
#include "svm_model_v4.h"
...
// ── Z-Score Normalization Constants (Wajib untuk SVM) ──
const float FEATURE_MEANS[4]  = { 0.39055566, 0.17166698, 0.26229811, 77.92584906 };
const float FEATURE_SCALES[4] = { 0.19170289, 0.08503130, 0.03005053, 10.50780374 };
...
void loop() {
  ...
  const float MEAN_IR_RATIO  = 0.38780654f;
  const float SCALE_IR_RATIO = 0.19271216f;

  const float MEAN_RED_RATIO  = 0.17118505f;
  const float SCALE_RED_RATIO = 0.08598578f;

  const float MEAN_CTR  = 0.26270748f;
  const float SCALE_CTR = 0.03046111f;

  const float MEAN_HR  = 77.86280374f;
  const float SCALE_HR = 10.45691381f;
  ...
  Serial.printf("✅ SQA PASSED! HR: %.1f, TimeCV: %.1f%%, AmpCV: %.3f\n", g_heartRate, g_time_cv, g_amp_cv);
      ...
      // --- NORMALISASI Z-SCORE UNTUK SVM ---
      float raw_features[4] = { g_irRatio, redRatio, g_ctr, g_heartRate };
      float norm_features[4];
      for(int i=0; i<4; i++) {
          norm_features[i] = (raw_features[i] - FEATURE_MEANS[i]) / FEATURE_SCALES[i];
      }

      // --- KLASIFIKASI TINYML ---
      unsigned long mlStart = millis();
      int prediction = svm.predict(norm_features);
      g_inferTime = millis() - mlStart;

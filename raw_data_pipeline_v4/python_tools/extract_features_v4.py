...
OUTPUT_DIR    = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'dataset_v4'))
OUTPUT_FILE   = os.path.join(OUTPUT_DIR, 'extracted_features_v4.csv')
...
def extract():
    ...
    for file in csv_files:
        ...
        window_size  = 1000
        # EKSPERIMEN: Ambil hingga 20 detik pertama (2 window)
        num_windows  = 2 if len(df) >= 2000 else (1 if len(df) >= 1000 else 0)
        ...
        for w in range(num_windows):
            ...
            if time_cv > 10.0:
                print(f"  -> [REJECT] {os.path.basename(file)} w{w+1}: Time CV {time_cv:.1f}%")
                continue

            # SQA Gate 2: Amplitude CV
            amps = ir_flt[peaks]
            ...
            if amp_cv > 0.37:
                print(f"  -> [REJECT] {os.path.basename(file)} w{w+1}: Amp CV {amp_cv*100:.1f}%")
                continue
    ...
    # ── Undersampling Round-Robin (max 10 per subjek) ──
    print("\n" + "=" * 65)
    print("UNDERSAMPLING (Maks 10 window per Subjek)")
    print("=" * 65)
    MAX_PER_SUBJECT = 10
    undersampled = []
    for subj, grp in df_out.groupby('subject_id'):
